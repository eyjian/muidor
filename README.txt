简单原理的极高性能分布式无单点高可用性唯一ID生成服务Uidor

涉及的MySQL库表，请参见master.cpp文件中的注释，在运行UidMaster之前，需要先创建一个库和三张表。
...
MOOON-Uidor各组成均为单进程无线程，成员间通讯采用UDP协议，以致实现轻巧但又高效。
...
工具说明：
1) UidorCli可用于测试UidorAgent，并可作为性能测试工具
2) MasterCli可用于测试Uidormaster，对于Uidormaster来说不需要考虑性能
...
4) Uidormaster的timeout值要小于UidorAgent的interval值，以便及时的将值更新到DB中，比如可以为interval值的一半，或6/10等
5) UidorAgent的steps值最好不小于10000，设置为10万会更佳，每重启一次agent进程，最多会浪费steps两倍的sequence，因此太大也不好。
