...
void print_transaction_id(const char* agent_nodes, bool polling)
{
    try
    {
        ...
        str = muidor.get_transaction_id("02%L%Y%M%D%m%7S");
        fprintf(stdout, "[A2]NO.: %s\n", str.c_str());
        str = muidor.get_transaction_id("02%L%Y%M%D%m%9S");
        fprintf(stdout, "[A3]NO.: %s\n\n", str.c_str());

        str = muidor.get_transaction_id("02%L%Y%M%D%m%8S%s", "##");
        fprintf(stdout, "[B1]NO.: %s\n", str.c_str());
        str = muidor.get_transaction_id("%s02%L%Y%M%D%m%8S", "##");
        fprintf(stdout, "[B2]NO.: %s\n\n", str.c_str());

        str = muidor.get_transaction_id("02%L%Y%M%D%m%8S%2X", 31);
        fprintf(stdout, "[C1]NO.: %s\n", str.c_str());
        str = muidor.get_transaction_id("02%L%Y%M%D%m%8S%2X", 1000);
        fprintf(stdout, "[C2]NO.: %s\n\n", str.c_str());

        // 批量取流水号
        std::vector<std::string> str_id_vec;
        muidor.get_transaction_id(num, &str_id_vec, "%3d%L%Y%M%D%H%S", 9);
        for (uint16_t i=0; i<num; ++i)
            fprintf(stdout, "[%d] %s\n", i, str_id_vec[i].c_str());
        fprintf(stdout, "\n");

        // 批量取4字节Seq
        uint32_t seq = muidor.get_unqi_seq(3);
        for (uint16_t i=0; i<num; ++i, ++seq)
            fprintf(stdout, "seq: %u\n", seq);
        seq = muidor.get_unqi_seq();
        fprintf(stdout, "seq: %u\n", seq);
        fprintf(stdout, "\n");

        // 批量取8字节ID
        std::vector<uint64_t> int_id_vec;
        muidor.get_local_uniq_id(num, &int_id_vec);
        for (uint16_t i=0; i<num; ++i)
        {
        ...
        for (int i=0; i<2; ++i)
        {
            ...
            muidor.get_label_and_seq(&label, &seq);
