...
// Usage1: uidor_stress uidor_agent_nodes
// Usage2: uidor_stress uidor_agent_nodes times
// Usage3: uidor_stress uidor_agent_nodes times concurrency
// Usage4: uidor_stress uidor_agent_nodes times concurrency poll
int main(int argc, char* argv[])
{
    ...
    fprintf(stdout, "uidor_agent_nodes: %s\n", agent_nodes);
...
void usage()
{
	fprintf(stderr, "Usage1: uidor_stress uidor_agent_nodes\n");
	fprintf(stderr, "Usage2: uidor_stress uidor_agent_nodes times\n");
	fprintf(stderr, "Usage3: uidor_stress uidor_agent_nodes times concurrency\n");
	fprintf(stderr, "Usage4: uidor_stress uidor_agent_nodes times concurrency poll\n");
}
...
void thread_proc(uint64_t times, const char* agent_nodes, bool polling)
{
    ...
    for (uint64_t i=0; i<times; ++i)
    {
        try
        {
            mooon::CUidor uidor(agent_nodes, timeout_milliseconds, retry_times, polling);
#if 1
            uint64_t uid = uidor.get_uniq_id();
#else
            uint64_t uid = uidor.get_local_uniq_id();
#endif
