...
bool CUidMaster::generate_labels()
{
        ...
        else
        {
            ...
            else
            {
                _mysql->update("INSERT INTO t_label_log(f_label,f_ip,f_event,f_time) VALUES (%s,\"%s\",%d,\"%s\")",
                        label_str.c_str(), ip_str.c_str(), LABEL_RENTED, time_str.c_str());
...
int CUidMaster::get_label() const
{
    try
    {
...
time_t CUidMaster::get_expire_time() const
{
    time_t  expire_time;
...
void CUidMaster::prepare_response_error(int errcode)
{
