...
CUidor::CUidor(const std::string& agent_nodes, uint32_t timeout_milliseconds, uint8_t retry_times, bool polling) throw (utils::CException)
    : _echo(ECHO_START), _agent_nodes(agent_nodes), _timeout_milliseconds(timeout_milliseconds), _retry_times(retry_times), _polling(polling), _udp_socket(NULL)
{
    ...
    for (std::multimap<std::string, std::string>::const_iterator iter=tokens.begin(); iter!=tokens.end(); ++iter)
    {
        ...
        {
            THROW_EXCEPTION("[Uidor] invalid port parameter", ERROR_PARAMETER);
        }
        ...
        {
            THROW_EXCEPTION("[Uidor] invalid IP parameter", ERROR_PARAMETER);
        }
...
uint8_t CUidor::get_label() throw (utils::CException, sys::CSyscallException)
{
    ...
    for (uint8_t retry=0; retry<_retry_times+1; ++retry)
    {
        ...
        try
        {
            ...
            if (bytes != sizeof(request))
            {
                THROW_SYSCALL_EXCEPTION(
                        utils::CStringUtils::format_string("[Uidor][%s] invalid size", net::to_string(agent_addr).c_str()),
                        bytes, "send_to");
            }
            ...
            {
                THROW_SYSCALL_EXCEPTION(
                        utils::CStringUtils::format_string("[Uidor][%s] invalid size", net::to_string(from_addr).c_str()),
                        bytes, "receive_from");
            }
            else if (memcmp(&agent_addr, &from_addr, sizeof(struct sockaddr_in)) != 0)
            {
                THROW_EXCEPTION(
                        utils::CStringUtils::format_string("[Uidor][%s][AGENT:%s] unexcepted response", net::to_string(from_addr).c_str(), net::to_string(agent_addr).c_str()),
                        ERROR_UNEXCEPTED);
            }
            else if (RESPONSE_ERROR == response->type)
            {
                THROW_EXCEPTION(
                        utils::CStringUtils::format_string("[Uidor][%s] store sequence block error: %s", net::to_string(from_addr).c_str(), response->str().c_str()),
                        static_cast<int>(response->value1.to_int()));
            }
            else if (response->type != RESPONSE_LABEL)
            {
                THROW_EXCEPTION(
                        utils::CStringUtils::format_string("[Uidor][%s] error response label: %s", net::to_string(from_addr).c_str(), response->str().c_str()),
                        response->type.to_int());
            }
            else if (response->echo.to_int() != echo)
            {
                THROW_EXCEPTION(
                        utils::CStringUtils::format_string("[Uidor][%s] mismatch response label: %s|%u", net::to_string(from_addr).c_str(), response->str().c_str(), echo),
                        ERROR_MISMATCH);
            }
            else
            {
                ...
                {
                    THROW_EXCEPTION(
                            utils::CStringUtils::format_string("[Uidor][%s] illegal response: %s|%u", net::to_string(from_addr).c_str(), response->str().c_str(), magic_),
                            ERROR_ILLEGAL);
                }
#endif // _CHECK_MAGIC_
                ...
                {
                    THROW_EXCEPTION(
                        utils::CStringUtils::format_string("[Uidor][%s] invalid label from master: %s", net::to_string(from_addr).c_str(), response->str().c_str()),
                        ERROR_INVALID_LABEL);
                }
        ...
        catch (sys::CSyscallException& ex)
        {
            if ((0 == _retry_times) || (retry+1 >= _retry_times))
            {
                ...
                THROW_SYSCALL_EXCEPTION(
                        utils::CStringUtils::format_string("[Uidor][%s] receive timeout", net::to_string(agent_addr).c_str()),
...
uint32_t CUidor::get_unqi_seq(uint16_t num) throw (utils::CException, sys::CSyscallException)
{
    const uint32_t echo = get_echo(_echo);
    char response_buffer[1 + sizeof(struct MessageHead)]; // 故意多出一字节，以过滤掉包大小不同的脏数据
    struct MessageHead* response = reinterpret_cast<struct MessageHead*>(response_buffer);
    ...
    for (uint8_t retry=0; retry<_retry_times+1; ++retry)
    {
        ...
        try
        {
            ...
            {
                THROW_EXCEPTION(
                        utils::CStringUtils::format_string("[Uidor][%s] error response sequence: %s", net::to_string(from_addr).c_str(), response->str().c_str()),
                        response->type.to_int());
            }
            else if (response->echo.to_int() != echo)
            {
                THROW_EXCEPTION(
                        utils::CStringUtils::format_string("[Uidor][%s] mismatch response sequence: %s|%u", net::to_string(from_addr).c_str(), response->str().c_str(), echo),
                        ERROR_MISMATCH);
            }
            else
            {
...
uint64_t CUidor::get_uniq_id(uint8_t user, uint64_t current_seconds) throw (utils::CException, sys::CSyscallException)
{
    const uint32_t echo = get_echo(_echo);
    char response_buffer[1 + sizeof(struct MessageHead)]; // 故意多出一字节，以过滤掉包大小不同的脏数据
    struct MessageHead* response = reinterpret_cast<struct MessageHead*>(response_buffer);
    ...
    for (uint8_t retry=0; retry<_retry_times+1; ++retry)
    {
        ...
        try
        {
            ...
            {
                THROW_EXCEPTION(
                        utils::CStringUtils::format_string("[Uidor][%s] error response id: %s", net::to_string(from_addr).c_str(), response->str().c_str()),
                        response->type.to_int());
            }
            else if (response->echo.to_int() != echo)
            {
                THROW_EXCEPTION(
                        utils::CStringUtils::format_string("[Uidor][%s] mismatch response id: %s|%u", net::to_string(from_addr).c_str(), response->str().c_str(), echo),
                        ERROR_MISMATCH);
            }
            else
            {
...
uint64_t CUidor::get_local_uniq_id(uint8_t user, uint64_t current_seconds) throw (utils::CException, sys::CSyscallException)
{
    uint8_t label = 0;
    uint32_t seq = 0;
...
void CUidor::get_local_uniq_id(uint16_t num, std::vector<uint64_t>* id_vec, uint8_t user, uint64_t current_seconds) throw (utils::CException, sys::CSyscallException)
{
    uint8_t label = 0;
    uint32_t seq = 0;
...
void CUidor::get_label_and_seq(uint8_t* label, uint32_t* seq, uint16_t num) throw (utils::CException, sys::CSyscallException)
{
    const uint32_t echo = get_echo(_echo);
    char response_buffer[1 + sizeof(struct MessageHead)]; // 故意多出一字节，以过滤掉包大小不同的脏数据
    struct MessageHead* response = reinterpret_cast<struct MessageHead*>(response_buffer);
    ...
    for (uint8_t retry=0; retry<_retry_times+1; ++retry)
    {
        ...
        try
        {
            ...
            {
                THROW_EXCEPTION(
                        utils::CStringUtils::format_string("[Uidor][%s] error response label and sequence: %s", net::to_string(from_addr).c_str(), response->str().c_str()),
                        response->type.to_int());
            }
            else if (response->echo.to_int() != echo)
            {
                THROW_EXCEPTION(
                        utils::CStringUtils::format_string("[Uidor][%s] mismatch response label and sequence: %s|%u", net::to_string(from_addr).c_str(), response->str().c_str(), echo),
                        ERROR_MISMATCH);
            }
            else
            {
...
std::string CUidor::get_transaction_id(const char* format, ...) throw (utils::CException, sys::CSyscallException)
{
    va_list ap;
    va_start(ap, format);
...
std::string CUidor::get_transaction_id(const char* format, va_list& va) throw (utils::CException, sys::CSyscallException)
{
...
void CUidor::get_transaction_id(uint16_t num, std::vector<std::string>* id_vec, const char* format, ...) throw (utils::CException, sys::CSyscallException)
{
    va_list ap;
    va_start(ap, format);
    utils::VaListHelper vlh(ap);
...
void CUidor::get_transaction_id(uint16_t num, std::vector<std::string>* id_vec, const char* format, va_list& va) throw (utils::CException, sys::CSyscallException)
{
    char *s;
    int m, width;
    uint8_t label;
    ...
    for (uint16_t i=0; i<num; ++i, ++seq)
    {
        ...
        while (*format_p != '\0')
        {
            ...
            else
            {
                ...
                {
                    // format error
                    THROW_EXCEPTION("[Uidor] invalid `format` parameter", ERROR_PARAMETER);
                }
                else
                {
...
const struct sockaddr_in& CUidor::pick_agent() const
{
